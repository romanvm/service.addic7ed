...
def download_subs(link, referrer, filename):
    ...
    try:
        ...
        _log('Unable to connect to addic7ed.com', xbmc.LOGERROR)
        dialog.notification(_string(32002), _string(32005), 'error')
    except DailyLimitError:
        dialog.notification(_string(32002), _string(32003), 'error', 3000)
        _log('Exceeded daily limit for subs downloads.', xbmc.LOGERROR)
    else:
        ...
        dialog.notification(_string(32000), _string(32001), _icon, 3000, False)
        _log('Subs downloaded.')


def search_subs(params):
    _log('Searching for subs...', xbmc.LOGNOTICE)
    ...
    if _addon.getSetting('use_filename') == 'true' or not now_played['showtitle']:
        ...
        try:
            ...
            _log('Filename {0} failed. Trying ListItem.Label...'.format(filename))
            try:
                filename = now_played['label']
                _log('Using filename: {0}'.format(filename))
                showname, season, episode = functions.parse_filename(filename)
            except ParseError:
                _log('Unable to determine episode data for {0}'.format(filename), xbmc.LOGERROR)
                dialog.notification(_string(32002), _string(32006), 'error', 3000)
                return
    else:
        ...
        _log(u'Using library metadata: {0} - {1}x{2}'.format(showname, season, episode))
    ...
    if query:
        ...
        try:
            ...
            _log('No subs found.', xbmc.LOGNOTICE)
        else:
            _log('Found subs for "{0}"'.format(query), xbmc.LOGNOTICE)
